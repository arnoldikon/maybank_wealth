...
= Users API
REST API to manage user management

The project **usersAPI** is a Bonita REST API extension for **Bonita 7.12.1** version  **community** written in `groovy`
...
[source,bash]
----
git clone https://your.github.com/usersAPI.git
cd usersAPI/
mvn package
----

The build should produce a zip archive under the `target/` folder named `usersAPI-1.0.0-SNAPSHOT.zip`
...
Take the built artifact `usersAPI-1.0.0-SNAPSHOT.zip` and upload it to your Bonita platform (see https://documentation.bonitasoft.com/bonita/${shortBonitaVersion}/rest-api-extensions#toc7)
